...
Do not implement, deploy, redrive, retry, or switch the live schedule as part
of the current Domain 1 governance workstream. Revisit only after the open
governance tracker tasks are complete, or if the tracker is explicitly updated
to grant a short maintenance exception.
...
- [ ] Add local tests with a fake Bedrock client that raises a timeout before
      returning a model response.
- [ ] Prove that the handler writes a failed-zone record for managed AI
      timeout.
- [ ] Prove that no `ai_insight` artifact is written on managed timeout.
- [ ] Prove that `PublishDashboardSnapshot` is not reached after managed
      timeout.
- [ ] Prove that deterministic merge behavior remains unchanged.
- [ ] Add explicit Bedrock client timeout configuration in the Lambda runtime.
- [ ] Rebuild the Lambda package locally.
- [ ] Run a Terraform plan only after the package is rebuilt.
- [ ] Apply only under explicit approval.
- [ ] Run at most one controlled smoke execution after approval.
- [ ] Keep schedule behavior as a separate explicit decision.
...
This item stays parked until the current governance tracker open items are
