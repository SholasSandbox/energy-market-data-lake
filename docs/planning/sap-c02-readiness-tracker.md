...
│   │   ├── Security Tooling Config recorder scope: in scope for next bounded implementation
│   │   ├── GuardDuty delegated-admin planning target
│   │   └── future Security Hub / OAM home if adopted
│   └── so-aws-admin / 054394900225
│       └── Decommission path: retire after read-only dependency checks and dependency resolution
│
...
- [ ] Under separate explicit approval, onboard the `Security Tooling`
...
- [ ] Under separate explicit approval, collect fresh GuardDuty prechange
  evidence and configure GuardDuty delegated administration in `Security
  Tooling` only if the live implementation boundary is approved.
