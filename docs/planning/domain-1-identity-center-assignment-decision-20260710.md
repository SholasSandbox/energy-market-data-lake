...
and the 2026-07-02 break-glass cleanup evidence. It does not claim a fresh live
inventory beyond those cited records.
...
|---|---|---|---|
| Normal management administration | Retain the existing normal `AdministratorAccess` path for `org-admin-principal`; do not create or assign `OrganizationAdmin` in this slice. | It preserves the known Organizations recovery path while the exact least-privilege action set is not yet proven. | A reviewed custom `OrganizationAdmin` policy, named owner, rollback procedure, and explicit change approval exist. |
...
Before any future assignment, capture a fresh read-only inventory of the IAM
Identity Center instance, permission-set names and policy attachments, and
assignment counts for each in-scope account. The control-specific change note
must then include:
