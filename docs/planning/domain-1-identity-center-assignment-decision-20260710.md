...
| `SecurityToolingAdmin` | The one-hour, Security Tooling-only custom permission set is live for the existing `security-tooling-admins` group. The temporary broad Security Tooling `AdministratorAccess` assignment was removed under separate approval. | This completed the least-privilege replacement while preserving management, archive, emergency, and read-only boundaries. Portal assumption, read-only checks, an idempotent GuardDuty write, Event History, organization-trail evidence, and post-removal fresh sessions are proven. | A new principal, account scope, policy action, session change, or group-scope change requires fresh precheck, validation, rollback planning, and separate approval. |
...
- custom least-privilege policy documents for `OrganizationAdmin` and
  `BreakGlassAdmin`;
- expansion of the Security Tooling routine administrator path beyond the
  proven custom policy, group, account, and Region boundary;
