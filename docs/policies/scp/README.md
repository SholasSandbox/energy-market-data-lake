...
They are **examples only**. They are not attached to any OU or account, and this
repository does not authorize live AWS Organizations changes.
...
| `deny-root-user-actions.example.json` | Reduce root-user blast radius. | Emergency root-use process is documented and tested. |
